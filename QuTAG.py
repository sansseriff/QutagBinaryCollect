...
class QuTAG:
    ...
    def __init__(self):
        ...
        #path = '../..'
        path = '/home/elsa/QUTAG-LX64-V1.5.2/'
        sodll = 'libtdcbase.so'
        usbdll = 'libftd3xx.so'

        test_text = "license.txt"

        #with open(os.path.join(path,test_text), 'r') as f:
        #    lines = f.readlines()
        #    print(lines[0])
        ...
        self.qutools_dll = ctypes.cdll.LoadLibrary(os.path.join(path,sodll))
        self.qutools_usbdll = ctypes.cdll.LoadLibrary(os.path.join(path,usbdll))
        #print(dir_wrapper)
    ...
    def declareAPI(self):
        ...
        self.qutools_dll.TDC_setChannelLink.argtypes = [ctypes.c_int32]
        self.qutools_dll.TDC_setChannelLink.restype = ctypes.c_int32
...
# Configure Channels ----------------------------------------------------------------
    ...
    #added in by Andrew, basewd on documentation of API
    def setChannelLink(self,enable):
        if enable:
            ena = 1
        else:
            ena = 0
        ans = self.qutools_dll.TDC_setChannelLink(ena)
        if (ans != 0):
            print("Error in TDC_setChannelLink: "+self.err_dict[ans])
        return ans
